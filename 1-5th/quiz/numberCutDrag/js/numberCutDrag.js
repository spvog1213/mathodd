...
function initNumber() {
	...
	var Num1 = parseInt(gameManager.TOTAL_ANSWER_ARRAY[0]),
	    ...
	    answerPosition = gameManager.TOTAL_ANSWER_ARRAY[gameManager.TOTAL_ANSWER_ARRAY.length - 1];

	numpadBg.setAttribute('id', 'numpadBg');

	bgCanvas.appendChild(numpadBg);
	Question1.setAttribute('style', 'top:158px; left: 426px;');

	switch (answerPosition) {

	case 'A':
		Question2.setAttribute('style', 'top:444px; left: 279px;');
		circleAnswer.setAttribute('style', 'top:444px; left: 570px;');
		break;
	case 'B':
		Question2.setAttribute('style', 'top:444px; left: 571px;');
		circleAnswer.setAttribute('style', 'top:444px; left: 279px');
