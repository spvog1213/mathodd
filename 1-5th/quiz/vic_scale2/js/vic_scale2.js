function initScene() {
	...
	appendImageElement('scaleBg', 'images/vic_scale_scale3.png', scaleContainer);
...
function answerObject() {
	...
	appendCircleElement('answerBox3', 'answerAll', document.getElementById('bgCanvas'));
	...
	answerObject_2.setAttribute('style', 'top:140px; left: 384px;');
	answerObject_3.setAttribute('style', 'top:140px; left: 556px;');
	...
	answerObject_3.setAttribute('candleValue', gameManager.CURRENT_ANSWER[2]);
...
function scaleCompareAnswer(dragObj) {
	...
	var testNum = parseInt(dragObj.getAttribute('answervalue'));
	
	console.log('~~~~' + answerTxt.innerHTML);	

	testNum= parseInt(answerTxt.innerHTML) + testNum; 

	answerTxt.innerHTML = testNum;

	

	if(gameManager.CURRENT_ANSWER[0] === answerValues || gameManager.CURRENT_ANSWER[1] === answerValues || gameManager.CURRENT_ANSWER[2] === answerValues){
	...
	answer3  = answerBox3.childNodes[0],
	dragObjId = dragObj.id;
	dragObjIdNum = dragObjId.split('_'),
	dragObjpositon = parseInt(dragObjIdNum[1]) + 1;
	...
	if((incorrect.length === 1 && correct.length === 2) || (incorrect.length > 1 && correct.length < 2)){
		...
		answerTxt.innerHTML = parseInt(answer1.getAttribute('answervalue')) + parseInt(answer2.getAttribute('answervalue')) + parseInt(answer3.getAttribute('answervalue'));

		setTimeout(function(){
			incorrectAnimation(answer1);
			incorrectAnimation(answer2);
			incorrectAnimation(answer3);

			choiceQuestionContainer.appendChild(answer1);
			choiceQuestionContainer.appendChild(answer2);
			choiceQuestionContainer.appendChild(answer3);
			...
			answer3.setAttribute('style','pointer-evets: auto; margin-left: 0px; display:none;');

			answer1.className = 'circle';
			answer2.className = 'circle';
			answer3.className = 'circle';
