...
function initObject() {
	...
	content.setAttribute('style','background:url("images/icecube_icecube_1.png") no-repeat;');

	for(var i in gameManager.quizConvertNumber){
		document.querySelector('div.question_'+i).setAttribute('style','top:'+top+'px; left:'+left+'px;');
		left += 115;
	}


	for(var i in gameManager.quizIndex){
		var hiddenElement = document.querySelector('div.question_'+gameManager.quizIndex[i]),
...
function questionArrayMake(val){
	...
	Quotient = parseInt(7 / max),
	remainder = parseInt(7 % max);
