...
function inithangerBar() {
	...
	circleAnswer.setAttribute('style','position:absolute; top:-211px; left:8px; width:252px; height:300px; z-index:10;');
...
function puzzleCompareAnswer(dragObj, arrayNum) {
	...
	for(i = 0; i < 1 ; i++){
		...
		if(parseInt(arrayNum) === 2) {
			...
			seeSawRabbitHand.setAttribute('style','position:absolute; z-index:40; left:620px; top:-149px;');
			dragObj.style.top = '4px';

		}else{
			appendImageElement('seeSawBearHand', 'images/seeSaw_bear_hand.png',seeSawAllBox,'seeSawBearHand');
			seeSawBearHand.setAttribute('style','position:absolute; z-index:40; left:26px; top:48px;');

			dragObj.style.top = '9px';
			dragObj.style.left = '20px';
		} 
