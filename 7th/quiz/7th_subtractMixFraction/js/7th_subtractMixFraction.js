...
function drawBgBoxes() {
	...
	if(gameManager.QUIZ_OPTION[2] === 'left') {
		var transformText = 'transform: rotateX(180deg) rotateZ(180deg);';
		bgBoxes.setAttribute('style', transformText + ' -ms-' + transformText + ' -webkit-' + transformText);
	}
}
...
function drawTopImgs() {
	var topContainer, sizerObj, img, bunsu, int, bunja, bunmo;
	topContainer = createElement('div', canvas, 'topContainer');

	// 크기, 위치 설정
	sizerObj = { top: 50, left: 180, width: 855, height: 155 };
	Object.keys(sizerObj).forEach(function(el) { topContainer.style[el] = sizerObj[el] + 'px'; });

	// 이미지 설정
	bunsu = gameManager.QUIZ_OPTION[0];
	int = bunsu[0];
	bunja = bunsu[1];
	bunmo = bunsu[2];
	for (var i = 0; i < int; i++) {
...
function drawMidImgs() {
	var midContainer, sizerObj, img, intLength, dragObj;
	midContainer = createElement('div', canvas, 'midContainer');
	dropArea = createElement('div', canvas, 'dropArea_0');

	// 크기, 위치 설정
	sizerObj = { top: 275, left: 150, width: 445, height: 155 };
	Object.keys(sizerObj).forEach(function(el) { midContainer.style[el] = sizerObj[el] + 'px'; });
	Object.keys(sizerObj).forEach(function(el) { dropArea.style[el] = sizerObj[el] + 'px'; });

	// 이미지 설정
	bunsu = gameManager.QUIZ_OPTION[1];
	int = bunsu[0];
	bunja = bunsu[1];
	bunmo = bunsu[2];
	for (var i = 0; i < int; i++) {
		img = createElement('img', midContainer);
		img.src = './images/' + gameManager.randomImg + '_' + bunmo + '_' + bunmo + '.png';
	}
	img = createElement('img', midContainer);
	img.src = './images/' + gameManager.randomImg + '_' + bunmo + '_' + bunja + '.png';
	...
	// 오른쪽, 왼쪽 여부에 따른 위치 설정
	if(gameManager.QUIZ_OPTION[2] === 'right') {
		midContainer.style.left = sizerObj.left + 'px';
		dropArea.style.left = (1220 - sizerObj.width - sizerObj.left) + 'px';
	} else {
		midContainer.style.left = (1220 - sizerObj.width - sizerObj.left) + 'px';
		dropArea.style.left = sizerObj.left + 'px';
	}
...
function drawPopup() {
	...
	if (gameManager.QUIZ_OPTION[2] === 'right') QSAll('.popup > span')[4].classList.add('boxed');
	else QSAll('.popup > span')[2].classList.add('boxed');
...
function boundingCircle(dragObj, x, y) {
	...
	if (dropCompare(dragObj, x, y)) {
		...
		if (idx < 2) {
			// dragObj가 통 과일일 경우
			if (gameManager.completeObjs.length < 3 && idx === 0) {
			...
			// dragObj가 조각 과일일 경우
			} else if (gameManager.completeObjs.length <= 3 && idx === 1 && gameManager.dabCount[1] !== 3) {

				// 조각 과일 갯수가 분모 갯수 - 1보다 적을 경우만
		...
		console.log('gameManager.completeObjs:', gameManager.completeObjs);
		console.log('gameManager.dabCount:', gameManager.dabCount);
	} else {
		log('>>>>> not bounding!');
