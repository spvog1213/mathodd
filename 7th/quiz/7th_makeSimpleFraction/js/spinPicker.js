...
SALTGAMER.SpinPicker = (function (cellWidth) {
	...
	onOrientationChange: function (e) {
		window.scrollTo(0, 0);
		this.swWrapper.style.left = window.innerWidth + window.pageXOffset + 'px';
	...
	onScroll: function (e) {
		...
		calculateSlotsWidth: function () {
			...
			for (var i = 0; i < div.length; i += 1) {
				//this.slotEl[i].slotHeight = div[i].offsetHeight;
				//this.slotEl[i].slotTop = div[i].offsetTop;
			}
		},

		create: function () {
			var i, l, out, maindiv, div;
			...
			for (l = 0; l < this.slotData.length; l += 1) {
				...
				for (i in this.slotData[l].values) {
					out += '<div class="numberDiv">' + this.slotData[l].values[i] + '</div>';
				}
				...
				maindiv.slotPosition = l;
				maindiv.slotXPosition = 0;
				//maindiv.slotHight = 0;
				// ul.slotWidth = 800+'px';
				maindiv.slotMaxScroll = -(cellWidth * (maindiv.childNodes.length-1));

				maindiv.style.webkitTransitionTimingFunction = 'cubic-bezier(0, 0, 0.2, 1)';

				this.slotEl.push(maindiv);

				//console.log(this.cellWidth,'?ˆëŠ”')
				if (this.slotData[l].defaultValue) {
					this.scrollToValue(l, this.slotData[l].defaultValue);
				}
				maindiv.style.width = QS('.numberDiv').clientWidth * maindiv.querySelectorAll('.numberDiv').length+'px';
		...
		selectOpacity: function (slot, idx) {
			if (slot !== undefined) {
				...
				var slots = QS('.swSlots').childNodes,
					numberDiv = QSAll('.numberDiv');

				for(var i=0; i<numberDiv.length; i++){
					numberDiv[i].style.opacity = '0.2';
				}
		...
		getSelectedValues: function () {
			...
			for (i in this.slotEl) {
				...
				if (this.slotEl[i].slotXPosition > 0) {
					this.setPosition(i, 0);
				} else if (this.slotEl[i].slotXPosition < this.slotEl[i].slotMaxScroll) {
					this.setPosition(i, this.slotEl[i].slotMaxScroll);
				}

				index = -Math.round(this.slotEl[i].slotXPosition / this.cellWidth);
		...
		setPosition: function (slot, pos) {
			...
			this.slotEl[slot].slotXPosition = pos;
			// this.slotEl[slot].style.webkitTransform = 'translate3d(' + pos + 'px,0, 0)';

			this.slotEl[slot].style.position = 'absolute';
			this.slotEl[slot].style.left = pos +'px';
		...
		scrollStart: function (e) {
			...
			for (var i = 0; i < this.slotEl.length; i++) {
				slotTop = this.slotEl[i].getBoundingClientRect().top;
				slotHight = this.slotEl[i].getBoundingClientRect().height;
				if (yPos > slotTop && yPos < slotTop + slotHight) {
					this.activeSlot = i;
					break;
				} else {
					this.activeSlot = null;
			...
			if(this.activeSlot !== null){
				...
				if (theTransform != this.slotEl[this.activeSlot].slotXPosition) {
					this.setPosition(this.activeSlot, theTransform);
				}

				this.startX = event.clientX;
				this.scrollStartX = this.slotEl[this.activeSlot].slotXPosition;
		...
		scrollMove: function (e) {
			var event = this.getSelectEvent(e),
				topDelta = event.clientX - this.startX;
			if (this.activeSlot !== null) {
				if (this.slotEl[this.activeSlot].slotXPosition > 0 || this.slotEl[this.activeSlot].slotXPosition < this.slotEl[this.activeSlot].slotMaxScroll) {
					topDelta /= 2;
				}
				this.setPosition(this.activeSlot, this.slotEl[this.activeSlot].slotXPosition + topDelta);
				this.startX = event.clientX;
		...
		scrollEnd: function (e) {
			...
			if (this.slotEl[this.activeSlot].slotXPosition > 0 || this.slotEl[this.activeSlot].slotXPosition < this.slotEl[this.activeSlot].slotMaxScroll) {
				...
				this.scrollTo(this.activeSlot, this.slotEl[this.activeSlot].slotXPosition > 0 ? 0 : this.slotEl[this.activeSlot].slotMaxScroll);
				return false;
			}

			var scrollDistance = this.slotEl[this.activeSlot].slotXPosition - this.scrollStartX;

			if (scrollDistance < this.cellWidth / 1.5 && scrollDistance > -this.cellWidth / 1.5) {
				if (this.slotEl[this.activeSlot].slotXPosition % this.cellWidth) {

					this.scrollTo(this.activeSlot, Math.round(this.slotEl[this.activeSlot].slotXPosition / this.cellWidth) * this.cellWidth, '100ms');
				}
			...
			var newPosition = this.slotEl[this.activeSlot].slotXPosition + newScrollDistance;

			if (newPosition > 0) {
				newPosition /= 2;
				newDuration /= 3;

				if (newPosition > this.swSlotWrapper.clientWidth / 4) {
					newPosition = this.swSlotWrapper.clientWidth / 4;
				}
				...
				if (newPosition < this.slotEl[this.activeSlot].slotMaxScroll - this.swSlotWrapper.clientWidth / 4) {
					newPosition = this.slotEl[this.activeSlot].slotMaxScroll - this.swSlotWrapper.clientWidth / 4;
				}
			} else {
				newPosition = Math.round(newPosition / this.cellWidth) * this.cellWidth;
			}
		...
		scrollTo: function (slotNum, dest, runtime) {
			...
			var idx = (Math.round(dest / this.cellWidth) == 0) ? 0 : Math.round(dest / this.cellWidth) * (-1);
			if(idx > this.slotEl[slotNum].childNodes.length-1){
				idx = this.slotEl[slotNum].childNodes.length-1
			}

			this.selectOpacity (slotNum, idx);
			if (this.slotEl[slotNum].slotXPosition > 0 || this.slotEl[slotNum].slotXPosition < this.slotEl[slotNum].slotMaxScroll) {
		...
		scrollToValue: function (slot, value) {
			...
			for (i in this.slotData[slot].values) {
				if (i == value) {
					xPos = count * this.cellWidth;
					this.setPosition(slot, xPos);
					break;
				}
		...
		backWithinBoundaries: function (e) {
			...
			this.scrollTo(e.target.slotPosition, e.target.slotXPosition > 0 ? 0 : e.target.slotMaxScroll, '150ms');
			return false;
		},
