function initScene() {
	...
	if(gameManager.quizText.length > 1){
		str = "../../images/common/" + gameManager.quizText[0] + "/" + gameManager.quizText[1] + ".png";
		questionBox.innerHTML = "<img src = "+ str +" class='question'/>"+gameManager.quizText[2];
	}else if(gameManager.quizText.length == 1){
		questionBox.innerHTML = '<div>'+gameManager.quizText+'</div>';
	}
...
function initObject(val) {
	...
	var flowerImg =['magnet_blue', 'magnet_mint', 'magnet_pink', 'magnet_purple' ,'magnet_red' ,'magnet_yellow'];
	var numArray = [0, 1, 2, 3, 4, 5];
	
	for(var index = 1; index < val.length; index++){
			randomNum = Math.round(Math.random()*(numArray.length-1));//0~4
			Array.prototype.removeAt = function (index) {
			    this.splice(index, 1);
			};
        
			str += '<div class="drag_'+index+'"  id="drag_'+index+'">'+
					'<img src="images/figure/'+flowerImg[numArray[randomNum]]+'.png">'+
					'<div class="dragText" style="font-size:'+dragFontSize+'px";>'+val[index]+'</div>'+
					'</div>';
        
		    numArray.removeAt(randomNum);//배열에서 제거
		
	}
/*	for(var index = 1; index < val.length; index++){
		if(val[index].length==2){
			str += '<div class="drag_'+index+'"  id="drag_'+index+'">'+
				   '<img src="images/'+(val[index])[0]+'/'+(val[index])[1]+'.png">'+
				   '</div>';
	 	}else if(val[index].length==3){//이미지위에 텍스트
	 		str += '<div class="drag_'+index+'"  id="drag_'+index+'">'+
				   '<img src="images/'+(val[index])[0]+'/'+(val[index])[1]+'.png">'+
				   '<div class="dragText" style="font-size:'+dragFontSize+'px";>'+(val[index])[2]+'</div>'+
				   '</div>';
	 	}else if(val[index].length==1){//텍스트
	 		str += '<div class="drag_'+index+'"  id="drag_'+index+'">'+
				   '<div class="dragText" style="font-size:'+dragFontSize+'px";>'+val[index]+'</div>'+
				   '</div>';
	 	}
	}*/
		dragObjContainer.innerHTML = str;
...
function appendQuiz(buttonType, val) {
	...
	for (var index = 1; index < parseInt(val.length/2)+1; index++) {
		...
		if(index % 2 === 0) dragObj.style.top = choiceTop + 'px';
		else dragObj.style.top = choiceTop + 30 + 'px';
	...
	for (var index = parseInt(val.length/2)+1; index < (val.length); index++) {
		var dragObj = document.querySelector('.drag_'+index);
		if(index % 2 === 1) dragObj.style.top = choiceTop + 'px';
		else dragObj.style.top = choiceTop + 30 + 'px';
		dragObj.style.left = choiceLeft + 'px';
		dragObj.setAttribute('answerValue',(val[index])[1]);
...
function appendObject(postion, left, top){
	...
	switch(postion.length){
		case 1: left = 560; break;
		case 2: left = 400; temp = 250; break;
		case 3: left = 300; temp = 250; break;
		case 4: left = 240; temp = 200; break;
		case 5: left = 220; temp = 150; break;
		case 6: left = 180; temp = 140; break;
	}

	for (var index = 0; index < postion.length; index++) {
		if(index % 2 === 0) postion[index].style.top = top + 30 + 'px';
		else postion[index].style.top = top + 'px';
		postion[index].style.left = left + 'px';
		left += temp;
	}
}
