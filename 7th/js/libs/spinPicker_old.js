...
SALTGAMER.SpinPicker = (function (cellHeight) {
	...
	var eventCheck = 'ontouchstart' in window || window.DocumentTouch && document instanceof DocumentTouch,
		isIE = (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (navigator.userAgent.toLowerCase().indexOf("msie") != -1),
		spinPicker = {
	...
	onScroll: function (e) {
		...
		setPosition: function (slot, pos) {
			...
			if(isIE) this.slotEl[slot].style.transform = 'translate3d(0, ' + pos + 'px, 0)';
			else this.slotEl[slot].style.webkitTransform = 'translate3d(0, ' + pos + 'px, 0)';
		},
		...
		scrollStart: function (e) {
			...
			if(this.activeSlot !== null){
				...
				if(isIE) {
					var theTransform = window.getComputedStyle(this.slotEl[this.activeSlot]).transform;
					theTransform = new MSCSSMatrix(theTransform).m42;
				} else {
